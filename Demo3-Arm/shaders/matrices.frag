...
void main(void)
{
	...
	vec4 param5 = texture2DRect(texSplineParams,texCoordParams+vec2(5,0));
	//.x:deformation_style
	//.y:spline_hardness
	//.z:spline_twist_adjust
	//.w:center_distort
	...
	float hardness		= param5.y;
	float twist_adjust	= param5.z;
	
	float a01 = clamp(a,0,1);
	
	float a_21 = a*2-1;
	float a_rot = sign(a_21) * (1-pow(1-abs(a_21),twist_adjust)) * 0.5 +0.5;
