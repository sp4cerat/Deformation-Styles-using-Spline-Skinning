...
//		printf("Fps:%d, %3.3f ms Light: %3.3f %3.3f %3.3f r %3.3f %3.3f %3.3f\n",fps,ms,pos.x,pos.y,pos.z,rot.x,rot.y,rot.z); 
		fps = 0;
		t1 = t2;
		tps = 0;
	}
	...
	*/
	...
	mx = 0.9*mx + GFX::mouseX*0.1;
	my = 0.9*my + GFX::mouseY*0.1;


	for (int c=0;c<num_characters;c++)
	{
		//float a = 3*0.5*sin(GFX::mouseX*4+rnd[c&7]*(float(c)*20/float(num_characters)));
		//float b = 6*0.5*sin(GFX::mouseY*4+rnd[c&7]*(float(c)*20/float(num_characters)));
		//if(c==0)
		//	setup_bind_pose( (GFX::mouseX-0.5)*3,(GFX::mouseY-0.5)*6 );
		//else
		//	setup_bind_pose( a , b );

		setup_bind_pose( (mx-0.5)*3 , (my-0.5)*3 );
