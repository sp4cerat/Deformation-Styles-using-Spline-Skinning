...
static Bmp deform_map("../media/textures/deform1.bmp");
static Bmp displacement_map_front("../media/style0/displacement_front.bmp");
static Bmp displacement_map_left_right("../media/style0/displacement_side.bmp");
static Bmp displacement_map_back("../media/style0/displacement_back.bmp");
...
ObjManager manager;
///////////////////////////////////////////
void draw_arc(vec3f p1,vec3f p2,vec3f p3)
{
	vec3f d1 = p2-p1;
	vec3f d2 = p3-p2;

	vec3f pos,pos_before;float a;

	for (a=0 ; a<1.05 ; a+= 0.05f)
	{
		float m1 = sin (a*0.5*M_PI);
		float m2 = 1-cos (a*0.5*M_PI);

		pos_before = pos ; pos = p1 + d1*m1 + d2*m2;

		if (a>0) GFX::Line(pos_before,pos,vec3f(0,0,0));
	}
	p1=p1+vec3f(6,0,0);
	p2=p2+vec3f(6,0,0);
	p3=p3+vec3f(6,0,0);
	...
	for (a=0 ; a<1.05 ; a+= 0.05f)
	{
		...
		pos_before = pos ; pos = p1*m1 + p2*m2 + p3*m3;
...
void setup_bind_pose(float x=0,float y=0)
{
	...
	vec3f b1( 0,  1+8.069+1-0.5, 0 );
	vec3f b2;
	vec3f b3( 0, -0+5.745-0.5, 0 );
/*
	vec3f b1( 0.049, 8.069, -0.172 );
	vec3f b2;
...
*/
	...
	b3.z+=0.1;
	b3.y+=fabs(float(x))+fabs(float(y));
//	b3.y+=fabs(float(x/2));
...
*/
	...
	spline_body.set ( b1,b2,b3,0,rota*2);
//	spline_body.set ( b1,b2,b3,0,0);
...
void DrawScene()
{
	...
	//	fbo.init( 512,512 );
		...
		*/	
		...
		static float *mem_spline_nbtpx= new float[sb_w*(sb_h+pose_h)*4];
		static float *mem_spline_nbtpy= new float[sb_w*(sb_h+pose_h)*4];
		static float *mem_spline_nbtpz= new float[sb_w*(sb_h+pose_h)*4];

		const int   spline_num_params = 6;
		static float mem_spline_params[sb_h*spline_num_params*4];

		for (int y=0;y<pose_h;y++)
		for (int x=0;x<sb_w;x++)
		{
			...
			if (y==pose_h-1)
			{
				...
				f_dx_dz_2.x = -cos(pos_x * 2*M_PI)*0.25+1.25;
				f_dx_dz_2.z = -sin(pos_x * 2*M_PI)*0.25;
				f_dx_dz_2.y =  0;

				f_dx_dz_2 = vec3f(1,0,0);

				f_dx_dz_3.x = cos(pos_x * 2*M_PI)*0.25+1.25;
		...
		tex.handle_nbtpx=  GFX::NewFloat16Tex(sb_w,sb_h+pose_h,(char*)mem_spline_nbtpx,true);
		tex.handle_nbtpy=  GFX::NewFloat16Tex(sb_w,sb_h+pose_h,(char*)mem_spline_nbtpy,true);
		tex.handle_nbtpz=  GFX::NewFloat16Tex(sb_w,sb_h+pose_h,(char*)mem_spline_nbtpz,true);
		tex.handle_params= -1;
		spline_tex = tex;
	...
	*/
	...
	mx = 0.8*mx + GFX::mouseX*0.2;
	my = 0.8*my + GFX::mouseY*0.2;

	for (int c=0;c<num_characters;c++)
	{
		float a = 3*0.5*sin(mx*4+rnd[c&7]*(float(c)*20/float(num_characters)));
		float b = 6*0.5*sin(my*4+rnd[c&7]*(float(c)*20/float(num_characters)));
		//if(c==0)
			setup_bind_pose( (mx-0.5)*3,(my-0.5)*6 );
		//else
		//	setup_bind_pose( a , b );
...
//	manager.load_obj ("../media/bar3.obj");
