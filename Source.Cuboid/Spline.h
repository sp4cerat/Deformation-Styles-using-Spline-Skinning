...
class Spline { public:
	...
	void get_at ( float a , bool rotate=true)
	{
		...
		float p=float(global_bend_adjust);//GFX::mouseY*5);
...
/*
	...
	{
		int ofs = y*tex.num_params*4; // 4 params, 4 floats per param
		...
		float angle = 2*(d1.angle(d2) / M_PI); angle = angle * angle;
