	...
	void draw(Material *materials, Spline* spline=NULL)
	{
		...
		if ( display_list.size() > 0 )
		{
			...
			for ( int instances_x = 0; instances_x < num_render/*num_characters*/; instances_x ++)
			{
				...
				glPushMatrix();
				glTranslatef(instances_x * 10,0,instances_y * 10);
		...
		else
		{
			...
			for ( i = 0 ; i < triangles.size() ; i++ )
			{
				...
				for ( std::size_t j = 0 ; j < 3 ; j++ )
				{
					...
					vec3f ww = weight_map.getPixel( t.x,t.y );
					//vec3f w (ww.x,ww.y,ww.z);
					vec3f w (1,0,0);
					w.clamp(0,1);
					...
					int weights=
					 int(float((w.x*255.0)))+
					 int(float((w.y*255.0)))*256+
					 int(float((w.z*255.0)))*256*256;
					...
					if ((w.y>w.x)&&(w.y>w.z))
					{
						weights=
						 int(float((w.y)*255))+
						 int(float((w.x)*255))*256+
						 int(float((w.z)*255))*256*256;
